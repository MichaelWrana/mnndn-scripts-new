...
def AnoaPad(list1, list2, padL, method):
    ...
    for j in range(0, 2):
        ...
        logger.info("Need to pad to %d packets."%topad) #gives a ± sum of originals+dummies
        while (lengths[j] < topad):
            curtime += AnoaTime([j, 0])
            if j == 0:
                paddings.append([curtime+starttime, 2])
            else:
                paddings.append([curtime+starttime, -2])
            lengths[j] += 1
    paddings = sorted(paddings, key = lambda x: x[0])
    list2.extend(paddings)
    logger.info(f"Number of dummies added in anoapad {added_dummies}")

def Anoa(list1, list2, parameters, timetoadd): #inputpacket, outputpacket, parameters
    #Does NOT do padding, because ambiguity set analysis. 
    #list1 WILL be modified! if necessary rewrite to tempify list1.
    starttime = list1[0][0]
    times = [starttime, starttime] #last time of outgoing packet, last time of incoming packet
    curtime = starttime
    lengths = [0, 0]
    datasize = DATASIZE
    method = 0
    ...
    #one dummy outgoing packet 
    curtime = times[0]
    list2.append([curtime + timetoadd, 2])
    logger.info(f"first dummy packet sent at: {times[0]}")

    #fill the gap with dummy incoming packets
    sent_dummy_incoming = int(OUT_TIME/IN_TIME)
    for _ in range(sent_dummy_incoming):
        times[1] += AnoaTime([1, 0])
        list2.append([times[1] + timetoadd, -2])
    logger.info(f"Last first dummy incoming packet sent at: {times[1]}")
    ...
    while (listind < len(list1)):
        ...
        if cursign == 0:
            if tosend == datasize:
                list2.append([curtime+timetoadd, DUMMYCODE])  
            else:
                list2.append([curtime+timetoadd, DATASIZE])  
        else:
            if tosend == datasize:
                list2.append([curtime+timetoadd, -DUMMYCODE]) 
            else:
...
if __name__ == '__main__':
    ...
    print("args.traces_path",args.traces_path)
    ...
    for site in tqdm(range(0, MON_SITE_NUM), desc="Sites"):
        site_traces = []
        for inst in tqdm(range(0, MON_INST_NUM), desc=f"Site {site} instances", leave=False):
            fname = f"site_{site}_trace_{inst}.txt"
            ...
            with open(os.path.join(args.traces_path, fname), "r") as f:
                ...
                for x in lines:
                    x = x.split("\t")
                    packets.append([float(x[0]) - starttime, int(x[1])]) #changes starttime to 0 and all other packets' appropriately
            
            list2 = [packets[0]] 
            parameters = [""]

            Anoa(packets, list2, parameters, starttime)
            ...
            if (SAVE_NPZ):
                signed_ts = []
                for x in list3:
                    signed_timestamp = np.float32(np.sign(x[1]) * x[0])
                    signed_ts.append(signed_timestamp)

                signed_ts = np.array(signed_ts, dtype=np.float32)
                site_traces.append(signed_ts)
        ...
        if (SAVE_NPZ):
            # Save npz for each site
            pickle_filename = os.path.join(foldout, f"website_{site}.npz")
            out_dict = {site: site_traces}
            with open(pickle_filename, "wb") as f:
                pickle.dump(out_dict, f)

            logger.info(f"Saved ONE pickle for website {site} with {len(site_traces)} instances.")
    ...
    #cmd format: cc_tamaraw.py <traces path>
    #to get defended perfect stable dataset: cc_tamaraw.py "../../txt_datasets/nodef/"  
    #to get defended perfect unstable dataset: cc_tamaraw.py "../../txt_datasets/nodef_unstable/"  
    #to get defended andana stable dataset: cc_tamaraw.py "../../txt_datasets/andana/"  
    #to get defended andana unstable dataset: cc_tamaraw.py "../../txt_datasets/andana_unstable/"  
