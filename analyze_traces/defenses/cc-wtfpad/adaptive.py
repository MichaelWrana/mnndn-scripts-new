...
class AdaptiveSimulator(object):
    ...
    def simulate(self, trace):
        ...
        # create a dictionary for -1:incoming and 1:outgoing. 
        ...
        for i, packet in enumerate(trace):
            ...
            # load the flow object based on the direction of the packet
            flow = flows[packet.direction]
            oppflow = flows[-packet.direction]
    ...
    def add_padding(self, i, trace, flow, on):
        ...
        # do not pad when in "waiting" state
        if flow.state == WAIT:
            return
        ...
        if not iat <= 0:
            if timeout < iat:
                ...
                if dummy.direction == OUT:
                    for i in range(sample_discrete_triangular(2,5,2)):
                        mirrored_dummy = Packet(dummy.timestamp + (i * 0.00001), IN, dummy=True)
                        insort_left(trace, mirrored_dummy)
