...
def regulator_download(target_trace):
    ...
    for i in range(added_packets):
        pkt_time = i*.1
        output_trace.append((pkt_time,-2))

    output_trace.append((target_trace[position],-1))
    ...
    while True:
        ...
        if(queue_length == 0 and padding_packets >= padding_budget):
            ...
            output_trace.append((current_time,-2))
            padding_packets += 1
        else:
            #real packet to send
...
def simulate(file_name):
    ...
    print(f"Processing {trace[0][3]}")

    if(len(trace) < 50): # failsafe for short traces to avoid crashing
        dump_path = args.output_path + str(file_name)
        shutil.copyfile(trace[0][3], dump_path)
        return
